...
function(cmaeit_find_pch_files HEADER_FILENAME SOURCE_FILENAME)
		...
		foreach(INTERNAL_HEADER_DIR ${INTERNAL_HEADER_DIRS})
			...
			foreach(INTERNAL_SOURCE_DIR ${INTERNAL_SOURCE_DIRS})
				...
				if(CMAKEIT_FIND_PCH_FILES_HEADER_FILENAME AND (NOT CMAKEIT_FIND_PCH_FILES_HEADER_DIR))
					...
					set(CMAKEIT_FIND_PCH_FILES_HEADER_OUTPUT_DIR ${INTERNAL_HEADER_DIR})
					...
					if(CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_VISUAL_C})
						...
						if(CMAKEIT_MODULE_PRIVATE_HEADER_DIR AND (CMAKEIT_FIND_PCH_FILES_HEADER_DIR STREQUAL ${CMAKEIT_MODULE_PRIVATE_HEADER_DIR}))
							set(CMAKEIT_FIND_PCH_FILES_HEADER_OUTPUT_DIR ${CMAKEIT_MODULE_OUTPUT_CONFIGURED_PRIVATE_HEADER_DIR})
						endif()

						if(CMAKEIT_MODULE_PUBLIC_HEADER_DIR AND (CMAKEIT_FIND_PCH_FILES_HEADER_DIR STREQUAL ${CMAKEIT_MODULE_PUBLIC_HEADER_DIR}))
							set(CMAKEIT_FIND_PCH_FILES_HEADER_OUTPUT_DIR ${CMAKEIT_MODULE_OUTPUT_CONFIGURED_PUBLIC_HEADER_DIR})
						endif()
	...
	if(CMAKEIT_FIND_PCH_FILES_HEADER_FILENAME)
		...
		set(CMAKEIT_MODULE_PCH_HEADER_OUTPUT_DIR ${CMAKEIT_FIND_PCH_FILES_HEADER_OUTPUT_DIR} PARENT_SCOPE)
		...
		unset(CMAKEIT_FIND_PCH_FILES_HEADER_OUTPUT_DIR)
...
#
# Parameters:
#   CMAKEIT_MODULE_UNIT_TEST (optional) - unit test target name. If not specified, configure current module target
#
function(cmakeit_target_apply_build_properties IS_UNIT_TEST UNIT_TEST_NAME)

	if(IS_UNIT_TEST AND UNIT_TEST_NAME)
		set(TARGET_NAME "${CMAKEIT_MODULE}_${UNIT_TEST_NAME}")
	else()

		set(TARGET_NAME "${CMAKEIT_MODULE}")
		set(IS_UNIT_TEST OFF)
	...
	if(IS_UNIT_TEST)
		...
		target_compile_definitions(${TARGET_NAME} PRIVATE "_UNIT_TEST")
	...
	if((NOT (CMAKEIT_BUILD_TYPE STREQUAL ${CMAKEIT_BUILD_TYPE_DEBUG})) OR (CYGWIN AND (CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_CLANG})))
	...
	if(CMAKEIT_DEPENDENCIES)
		
		foreach(INTERNAL_CMAKEIT_DEPENDENCY ${CMAKEIT_DEPENDENCIES})
			target_link_libraries(${TARGET_NAME} ${INTERNAL_CMAKEIT_DEPENDENCY})
		endforeach()

		unset(INTERNAL_CMAKEIT_DEPENDENCY)

	endif()

	if(IS_UNIT_TEST AND (CMAKEIT_MODULE_TYPE STREQUAL ${CMAKEIT_MODULE_TYPE_LIBRARY}))
	...
	if(CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_VISUAL_C})
		...
		if((CMAKEIT_BUILD_TYPE STREQUAL ${CMAKEIT_BUILD_TYPE_MINSIZEREAL}) OR (CMAKEIT_BUILD_TYPE STREQUAL ${CMAKEIT_BUILD_TYPE_RELEASE}) OR (CMAKEIT_BUILD_TYPE STREQUAL ${CMAKEIT_BUILD_TYPE_RELWITHDEBINFO}))
		...
		if(IS_UNIT_TEST OR ((CMAKEIT_MODULE_TYPE STREQUAL ${CMAKEIT_MODULE_TYPE_APPLICATION}) AND (CMAKEIT_MODULE_SUBTYPE STREQUAL ${CMAKEIT_MODULE_SUBTYPE_CONSOLE})))
	...
	elseif((CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_CLANG}) OR (CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_GCC}))
		...
		if(MINGW)
			...
			if((CMAKEIT_MODULE_TYPE STREQUAL ${CMAKEIT_MODULE_TYPE_APPLICATION}) AND (CMAKEIT_MODULE_SUBTYPE STREQUAL ${CMAKEIT_MODULE_SUBTYPE_UI}))
		...
		if((NOT IS_UNIT_TEST) AND ((CMAKEIT_MODULE_TYPE STREQUAL ${CMAKEIT_MODULE_TYPE_LIBRARY}) AND (CMAKEIT_MODULE_SUBTYPE STREQUAL ${CMAKEIT_MODULE_SUBTYPE_SHARED})))
	...
	if((NOT IS_UNIT_TEST) AND (NOT CMAKEIT_MODULE_NO_PCH) AND (CMAKEIT_MODULE_PCH_HEADER_FILENAME))
		...
		if((CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_VISUAL_C}) AND CMAKEIT_MODULE_PCH_SOURCE_FILENAME)
			...
			set(TARGET_MODULE_PCH_HEADER_OUTPUT_FILENAME "${CMAKEIT_MODULE_PCH_HEADER_OUTPUT_DIR}/${CMAKEIT_MODULE_PCH_HEADER_FILENAME_RELATIVE}.${CMAKEIT_COMPILER_PCH_SUFFIX}")
			...
			foreach(TARGET_MODULE_SOURCE_FILE ${CMAKEIT_MODULE_SOURCE_FILES})
				...
				if(NOT (TARGET_MODULE_SOURCE_FILE STREQUAL ${CMAKEIT_MODULE_PCH_SOURCE_FILENAME}))
		...
		elseif((CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_CLANG}) OR (CMAKEIT_COMPILER STREQUAL ${CMAKEIT_COMPILER_GCC}))
			...
			elseif((CMAKEIT_BUILD_TYPE STREQUAL ${CMAKEIT_BUILD_TYPE_MINSIZEREAL}) OR (CMAKEIT_BUILD_TYPE STREQUAL ${CMAKEIT_BUILD_TYPE_RELEASE}) OR (CMAKEIT_BUILD_TYPE STREQUAL ${CMAKEIT_BUILD_TYPE_RELWITHDEBINFO}))
